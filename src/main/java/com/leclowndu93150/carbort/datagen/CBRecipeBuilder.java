...
public class CBRecipeBuilder<T extends Recipe<?>> implements RecipeBuilder {
    private final T recipe;

    private CBRecipeBuilder(T recipe) {
        this.recipe = recipe;
    }

    public static <T extends Recipe<?>> CBRecipeBuilder<T> of(T recipe) {
        return new CBRecipeBuilder<>(recipe);
    }
    ...
    public void save(RecipeOutput recipeOutput) {
        ...
        save(recipeOutput, ResourceLocation.fromNamespaceAndPath(Carbort.MODID, recipe.getType().toString().split(":")[1] + "/" + builder));
    }
