...
public final class RecipeUtils {
    ...
    public static @Nullable List<ItemStack> itemsMatch(List<ItemStack> stacks, List<IngredientWithCount> ingredients) {
        List<IngredientWithCount> remainingIngredients = new ArrayList<>(ingredients);
        List<ItemStack> remainingStacks = new ArrayList<>(stacks);

        for (int i = 0; i < ingredients.size(); i++) {
            IngredientWithCount ingredient = ingredients.get(i);
            if (!remainingIngredients.contains(ingredient) || remainingIngredients.get(i).ingredient().isEmpty()) continue;

            for (ItemStack stack : stacks) {
                if (remainingStacks.contains(stack) && ingredient.ingredient().test(stack)) {
                    int countDiff = Math.max(0, ingredient.count() - stack.getCount());
                    remainingIngredients.set(i, countDiff != 0 ? new IngredientWithCount(ingredient.ingredient(), countDiff) : IngredientWithCount.EMPTY);
                    remainingStacks.set(i, countDiff != 0 ? stack.copyWithCount(countDiff) : ItemStack.EMPTY);
                }
            }
        }

        if (remainingIngredients.stream().filter(i -> !i.ingredient().isEmpty()).toList().isEmpty()) {
            return remainingStacks.stream().filter(i -> !i.isEmpty()).toList();
        }
        return null;
    }

    public static @Nullable Map<BlockPos, ItemStack> itemsMatch(Map<BlockPos, ItemStack> stacks, List<IngredientWithCount> ingredients) {
        List<IngredientWithCount> remainingIngredients = new ArrayList<>(ingredients);
        Map<BlockPos, ItemStack> remainingStacks = new HashMap<>(stacks);

        for (int i = 0; i < ingredients.size(); i++) {
            IngredientWithCount ingredient = ingredients.get(i);
            if (!remainingIngredients.contains(ingredient) || remainingIngredients.get(i).ingredient().isEmpty()) continue;

            for (Map.Entry<BlockPos, ItemStack> stack : stacks.entrySet()) {
                if (remainingStacks.containsValue(stack.getValue()) && remainingStacks.containsKey(stack.getKey()) && ingredient.ingredient().test(stack.getValue())) {
                    int countDiff = Math.max(0, stack.getValue().getCount() - ingredient.count());
                    remainingIngredients.set(i, countDiff != 0 ? new IngredientWithCount(ingredient.ingredient(), countDiff) : IngredientWithCount.EMPTY);
                    remainingStacks.put(stack.getKey(), countDiff != 0 ? stack.getValue().copyWithCount(countDiff) : ItemStack.EMPTY);
                }
            }
        }

        if (remainingIngredients.stream().filter(i -> !i.ingredient().isEmpty()).toList().isEmpty()) {
            return remainingStacks.entrySet().stream().filter(i -> !i.getValue().isEmpty()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        }
        return null;
    }
}